          ...
          <div className="flex space-x-1 dark:bg-customDark bg-gray-100 p-1 rounded-md">
            {items.map((item) => (
              <div
                ...
                className={`relative cursor-pointer px-4 py-1 rounded-sm  ${activeTab === item ? 'bg-gray-200 dark:bg-customDark' : 'text-gray-500'
                  }`}
              >
                {item}
              </div>
      ...
      <div className="w-full">
        <div className="w-full">
          {React.Children.map(children, (child, index) =>
            React.isValidElement(child) && child.props.label === activeTab ? (
...
const TabComponent = ({ children }: { children: React.ReactNode }) => {
  return <div className='border-2 border-gray-100 dark:border-customDark rounded-2xl overflow-hidden'>
